...
class Solution {
    public List<Integer> longestCommomSubsequence(int[][] arrays) {
        // Use a simple map to get count of each element from each array
        // if count == number of array, add it to list
        ...
        for(int i = 0; i < arrays.length; i++){
            ...
            for(int a : arr){
                if(i == arrays.length - 1){
                    if(map.containsKey(a) && map.get(a) == arrays.length - 1) resList.add(a);
                }
                else map.put(a, map.containsKey(a) ? map.get(a) + 1 : 1 );
            }
        }
        return resList;
    }
